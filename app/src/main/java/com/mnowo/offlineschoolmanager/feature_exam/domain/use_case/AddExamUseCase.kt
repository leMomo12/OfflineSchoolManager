...
class AddExamUseCase @Inject constructor(
    ...
    operator fun invoke(exam: Exam): Flow<Resource<ExamResult>> = flow<Resource<ExamResult>> {
        when (ExamValidation.validateExam(exam = exam)) {
            ToDoResult.EmptyDescription -> emit(
                Resource.Error<ExamResult>(
                    message = "",
                    data = ExamResult.EmptyDescription
                )
            )
            ToDoResult.EmptyTitle -> emit(
                Resource.Error<ExamResult>(
                    message = "",
                    data = ExamResult.EmptyTitle
                )
            )
            ToDoResult.SubjectNotPicked -> emit(
                Resource.Error<ExamResult>(
                    message = "",
                    data = ExamResult.SubjectNotPicked
                )
            )
            ToDoResult.Success -> {
                repository.addExam(exam = exam)
                emit(Resource.Success<ExamResult>(data = ExamResult.Success))
            }
        }
